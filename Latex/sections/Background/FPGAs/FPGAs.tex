...
%It may be intuitive to model FPGAs as a variant of a netlist, and then use placement- and routing technologies to obtain a placement of a virtual FPGA on a concrete FPGA and with that an emulation. However, by looking at lower abstraction levels than FPGA hardware, e.g. logic gate levels, the problem of finding an emulation becomes much more complex. Different FPGA designs may implement the same functionality (e.g. a LUT) using different combinations of gates and transistors. To find an emulation that exhibits the same functionality, we would thus have to find a mapping from sets of interconnected logic gates to sets of interconnected logic gates that are logically equivalent instead of finding a mapping of components that serve the same function (e.g. LUT to LUT). This means that we have $O(2^n!)$ possible mappings from virtual FPGA to concrete FPGA instead of $O(n!)$, which is computationally infeasible\footnote{To illustrate this complexity: adding a $100$th component to a $99$-component FPGA would multiply the search space by $10^{10^{32}}$ instead of by $100$.}.
...
\begin{tikzpicture}
    ...
    \draw (10, 0) -- (10, 2);
    \draw[thick,->] (6.8,0.5) node[anchor=north west] {in\_2} -- (8,0.5);
    \draw[thick,->] (6.8,1.5) node[anchor=north west] {in\_1} -- (8,1.5);
    
    \draw[thick,->] (11,1)  -- (12,1) node[anchor=north west] {out\_1};
    
    \draw[draw=white] (9,1.5)        node[anchor=south] {00};
    \draw[draw=white] (9,1)          node[anchor=south] {01};
    \draw[draw=white] (9,0.5)        node[anchor=south] {10};
    \draw[draw=white] (9,0)          node[anchor=south] {11};
    
    
    \draw[draw=white] (10.5,1.5)         node[anchor=south] {$y_0$};
    \draw[draw=white] (10.5,1)           node[anchor=south] {$y_1$};
    \draw[draw=white] (10.5,0.5)         node[anchor=south] {$y_2$};
    \draw[draw=white] (10.5,0)           node[anchor=south] {$y_3$};
    
    
    \draw[vecArrow] (5.5,1) -- (6.5,1);

\end{tikzpicture}
    \caption{Emulation of an FPGA: the programming of the virtual FPGA is reflected in the programming of the actual FPGA. In this example, $y_i$ takes the value of $x_i$. A less direct mapping could occur in emulation when, for example, a MUX is emulated by a LUT, or the inputs have a different permutation.}
    \label{fig:emulation}
\end{figure}
