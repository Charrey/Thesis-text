...
\color{purple}
\If{contraction} {
	$G_S, \mathit{chains} \longleftarrow \mathit{contract}(G_S)$ \color{black}\tcp*[h]{sec. \ref{sec:contraction}}\color{purple}
}
\color{black}
...
\If{$\mathit{hasUnmatchedEdge}()$} {
	$(e_S, p_T) \longleftarrow \mathit{GetNextEdgePathPair}()$\;
	\If(\tcp*[h]{sec. \ref{sec:unintendedcurrent-path}}){{\color{purple}$\mathit{isUnnecessarilyLong}(p_T) \lor$} \tcp*[h]{sec. \ref{sec:refusinglongerpaths}} \newline $\mathit{wouldPrune}${\color{purple}-ext}$(\mathit{vmap}, \mathit{emap} \cup \{(e_S, p_T)\}) \lor$\tcp*[h]{See Algorithm \ref{algorithm:wouldPrune-ext}}\newline{\color{purple}$(\mathit{contraction} \land \lnot \mathit{chainsCompatible}(\mathit{emap}, \mathit{from}(e_S), \mathit{to}(e_S), p_T)) \lor$}\tcp*[h]{sec. \ref{sec:contraction}}\newline{\color{purple}$\mathit{unconfigurableCover}(\mathit{intermediate}(p_T)) \cap (\mathit{emap} \cup \mathit{vmap}) \neq \emptyset$}} {
  		\textbf{continue}\;
  	}\color{purple}
	$V_T \longleftarrow V_T \setminus \mathit{unconfigurableCover}(\mathit{intermediate}(p_T))$\color{black}\tcp*[h]{sec. \ref{sec:unintendedcurrent-path}}\;  	
  	
  	$\mathit{emap} \longleftarrow \mathit{emap} \cup \{(e_S, p_T)\}$\;
  	$\mathit{found} \longleftarrow \mathit{ndSHD2} ${\color{purple}-ext}$(\mathit{vmap}, \mathit{emap})$\;
  \If{found} {
  	\Return true\;
  }
  $\mathit{emap} \longleftarrow \mathit{emap} \cap \{(e_S, p_T)\}$\;\color{purple}
  $V_T \longleftarrow V_T \cup \mathit{unconfigurableCover}(\mathit{intermediate}(p_T))$\color{black}  \tcp*[h]{sec. \ref{sec:unintendedcurrent-path}}\;	
} \Else {
	$(v_S, v_T) \longleftarrow \mathit{GetNextNodePair}()$\;
	\If(\tcp*[h]{sec. \ref{sec:unintendedcurrent-vertex}}){$\mathit{wouldPrune}${\color{purple}-ext}$(\mathit{vmap} \cup \{(v_S, v_T)\}, \mathit{emap})\lor$\tcp*[h]{See Algorithm \ref{algorithm:wouldPrune-ext}}\newline\color{purple}$\mathit{unconfigurableCover}(v_T) \cap (\mathit{vmap} \cup \mathit{emap}) \neq \emptyset$} {
  		\textbf{continue}\;
  	}
  	\color{purple}$V_T \longleftarrow V_T \setminus \mathit{unconfigurableCover}(v_T)$\color{black}\tcp*[h]{sec. \ref{sec:unintendedcurrent-vertex}}\;  
  	$\mathit{vmap} \longleftarrow \mathit{vmap} \cup \{(v_S, v_T)\}$\;
  	$\mathit{found} \longleftarrow \mathit{ndSHD2}${\color{purple}-ext}$(\mathit{vmap}, \mathit{emap})$\;
  \If{found} {
  	\Return true\;
  	...
  	$\mathit{vmap} \longleftarrow \mathit{vmap} \cap \{(v_S, v_T)\}$\;
  	\color{purple}$V_T \longleftarrow V_T \cup \mathit{unconfigurableCover}(v_T)$\color{black}  \tcp*[h]{sec. \ref{sec:unintendedcurrent-vertex}}\;	
}
